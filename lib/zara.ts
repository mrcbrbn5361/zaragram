...
import { ProductSnapshot, ProductTrackingRequest } from './types';

function hashSeed(value: string) {
  return value.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
}

export function inferReference(url: string) {
...
export function normalizeZaraProductUrl(url: string, marketCode: string) {
...
function buildSnapshot(input: ProductTrackingRequest, compareMarketCode?: string): ProductSnapshot {
  ...
  const normalizedUrl = normalizeZaraProductUrl(input.productUrl, marketCode);
...
export async function fetchPrimarySnapshot(input: ProductTrackingRequest) {
  return buildSnapshot(input);
}

export async function fetchComparisonSnapshots(input: ProductTrackingRequest) {
  const market = marketMap.get(input.marketCode);
  const comparisonMarkets = zaraMarkets

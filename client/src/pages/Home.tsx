...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main className="layout">
      ...
      <section className="content" id="overview"><div className="page-heading"><div><p className="eyebrow">FİYAT TAKİP MERKEZİ</p><h1>Fiyat değişimlerini<br /><span>kaçırmayın.</span></h1><p className="subheading">Ürün bağlantısını ekleyin. Telegram bağlıysa anlık mesaj, değilse bu tarayıcı üzerinden bildirim alın.</p></div><div className="heading-meta"><span className="live-dot" />{paused ? "Tarama duraklatıldı" : "Tarama hazır"}<br /><small>{tracks.length} aktif ürün</small></div></div>
        ...
        <div className="stats-grid"><div className="stat-card"><span>AKTİF TAKİPLER</span><strong>{tracks.length}</strong><small><Link2 size={13} /> Site hesabında saklanır</small></div><div className="stat-card"><span>İZLENEN MARKETLER</span><strong>{new Set(tracks.map((item) => item.market)).size}</strong><small><Globe2 size={13} /> Seçtiğiniz marketler</small></div><div className="stat-card"><span>FİYAT DEĞİŞİMİ</span><strong>{tracks.filter((item) => item.change !== "—").length}</strong><small><TrendingDown size={13} /> Gerçek tarama sonucu</small></div><div className="stat-card accent"><span>BİLDİRİM KANALI</span><strong>{telegramConnected ? "Telegram" : "Site"}</strong><small><Bell size={13} /> {telegramConnected ? "Telegram hazır" : "İzin bekliyor"}</small></div></div>
